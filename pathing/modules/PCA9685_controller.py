...
class PWMController:
    ...
    def _calculate_inputs(self):
        ...
        for config in self.channel_configs.values():
            if config.input_channel is not None:
                input_channels.add(config.input_channel)
        if self.pump_config and self.pump_config.input_channel is not None:
            input_channels.add(self.pump_config.input_channel)
    ...
    def update_values(self, raw_values: List[float]):
        """Update channel values - simple and direct."""
        # Rate monitoring signal
        if not self.skip_rate_checking:
            self.input_event.set()
            if not self.is_safe_state:
                # Skip output - system not ready
                return
        ...
        for name, config in self.channel_configs.items():
            if config.input_channel is None or config.input_channel >= len(raw_values):
                continue

            # Get and clamp input
            value = max(-1, min(raw_values[config.input_channel], 1))

            # Apply deadzone
            if abs(value) < config.deadzone_threshold:
                value = 0.0

            # Store value
            self.values[config.output_channel] = value

            # Update pump sum
            if config.affects_pump:
                self.pump_variable_sum += abs(value)
...
if __name__ == "__main__":
    try:
        controller = PWMController("config.yaml", input_rate_threshold=20)
        print("Controller initialized. Press Ctrl+C to exit...")

        while True:
            # Example input values
            test_values = [0.5, -0.3, 0.0, 0.8]
            controller.update_values(test_values)
