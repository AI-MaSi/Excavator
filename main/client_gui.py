...
class ExcavatorGUI:
    ...
    # Default Y position (now user-adjustable via slider)
    Y_POSITION = 0.0
    ...
    def _create_widgets(self):
        ...
        # Y-position slider (above rotation)
        y_slider_frame = ttk.Frame(info_frame)
        y_slider_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(6, 0))
        ttk.Label(y_slider_frame, text="Y Position (m):", font=('Arial', 10)).grid(row=0, column=0, sticky=tk.W)
        self.y_slider = ttk.Scale(y_slider_frame, from_=-0.30, to=0.30, orient=tk.HORIZONTAL,
                                   command=self._on_y_change)
        self.y_slider.set(self.Y_POSITION)
        self.y_slider.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(2, 0))
        y_slider_frame.columnconfigure(0, weight=1)

        # Center Y button
        y_center_frame = ttk.Frame(y_slider_frame)
        y_center_frame.grid(row=2, column=0, pady=(5, 0))
        self.center_y_btn = ttk.Button(y_center_frame, text="Center Y",
                                       command=self._center_y)
        self.center_y_btn.pack()

        # Rotation slider (wider now) — placed under Y slider
        rot_slider_frame = ttk.Frame(info_frame)
        rot_slider_frame.grid(row=2, column=0, sticky=(tk.W, tk.E), pady=(10, 0))
    ...
    def _copy_command(self):
        ...
        if abs(self.current_rot_y) < 0.1:
            command = f"python set_pose.py {self.current_x:.3f} {self.current_y:.3f} {self.current_z:.3f}"
        else:
            command = f"python set_pose.py {self.current_x:.3f} {self.current_y:.3f} {self.current_z:.3f}  # Rot: {self.current_rot_y:.1f}°"
    ...
    def _reset_position(self):
        ...
        self.rot_slider.set(0.0)
        try:
            self.y_slider.set(self.DEFAULT_Y)
        except Exception:
            pass
        self._draw_dot()
        self._update_display()
